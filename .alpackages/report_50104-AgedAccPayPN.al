report 50104 "Aged Accounts Payable PN"
{
    DefaultLayout = RDLC;
    RDLCLayout = './AgedAccountsPayablePN.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'Aged Accounts Payable (With PN)';
        ...
        dataitem(Vendor; Vendor)
        {
            ...
            dataitem(CurrencyLoop; "Integer")
            {
                ...
                dataitem(TempVendortLedgEntryLoop; "Integer")
                {
                    ...
                    //#001<<
                    column(PNNO; PNList(VendorLedgEntryEndingDate."Document No."))
                    {
                        AutoFormatExpression = CurrencyCode;
                        AutoFormatType = 1;
                    }
    ...
    var
        ...
        recPurCRLine: Record "Purch. Cr. Memo Line";
        txtPNList: Text;
        txtTempStr: Text;
    begin
        recPurInvLine.Reset();
        recPurInvLine.SetRange("Document No.", docNo);
        if recPurInvLine.FindSet() then begin
            repeat
                if recPurInvLine."Shortcut Dimension 1 Code" <> '' then begin
                    txtTempStr := format(recPurInvLine."Shortcut Dimension 1 Code");
            ...
            until recPurInvLine.Next() = 0;
        end;

        recPurCRLine.Reset();
        recPurCRLine.SetRange("Document No.", docNo);
        if recPurCRLine.FindSet() then begin
            repeat
                if recPurCRLine."Shortcut Dimension 1 Code" <> '' then begin
                    txtTempStr := format(recPurCRLine."Shortcut Dimension 1 Code");
                    if StrLen(txtPNList) = 0 then
                        txtPNList := txtTempStr
                    else
                        if StrPos(txtPNList, txtTempStr) = 0 then begin
