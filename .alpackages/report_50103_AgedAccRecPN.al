report 50103 "Aged Accounts Receivable PN"
//#001 - Add PN No. files in report - FCH 20220104
//#002 - Add program to get PN No. list - FCH 20220104
{
    DefaultLayout = RDLC;
    RDLCLayout = './AgedAccountsReceivablePN.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'Aged Accounts Receivable (with PN)';
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(TempCustLedgEntryLoop; "Integer")
                    {
                        ...
                        //#001<<
                        column(PNNO; PNList(CustLedgEntryEndingDate."Document No."))
                        {
                            AutoFormatExpression = CurrencyCode;
                            AutoFormatType = 1;
                        }
    ...
    //#002<<
    local procedure PNList(docNo: Code[20]): text
    var
        recSaleInvLine: Record "Sales Invoice Line";
        recSaleCRLine: Record "Sales Cr.Memo Line";
        txtPNList: Text;
        txtTempStr: Text;
    begin

        recSaleInvLine.Reset();
        recSaleInvLine.SetRange("Document No.", docNo);
        if recSaleInvLine.FindSet() then begin
            repeat
                if recSaleInvLine."Shortcut Dimension 1 Code" <> '' then begin
                    txtTempStr := format(recSaleInvLine."Shortcut Dimension 1 Code");
                    if StrLen(txtPNList) = 0 then
                        txtPNList := txtTempStr
                    else
                        if StrPos(txtPNList, txtTempStr) = 0 then begin
                            txtPNList := txtPNList + ',' + txtTempStr
                        end
                end;
        ...
        recSaleCRLine.Reset();
        recSaleCRLine.SetRange("Document No.", docNo);
        if recSaleCRLine.FindSet() then begin
            repeat
                if recSaleCRLine."Shortcut Dimension 1 Code" <> '' then begin
                    txtTempStr := format(recSaleCRLine."Shortcut Dimension 1 Code");
                    if StrLen(txtPNList) = 0 then
                        txtPNList := txtTempStr
                    else
                        if StrPos(txtPNList, txtTempStr) = 0 then begin
